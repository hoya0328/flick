...
현재 외부 베타에서는 이메일 로그인, 취향 설정, 실시간 추천·검색, 영화 상세, 보고 싶어요 저장과 Light/Core 감상 기록까지 사용할 수 있습니다. 저장소의 최신 개발본은 완료 기록의 `나만 보기`/`전체 공개` 선택, Gemini 기반 Core 연계 질문과 Q5 답변 기반 리뷰 초안 테스트를 추가했으며, 아직 외부 베타에는 재배포하지 않았습니다. 작성 중인 초안은 공개 선택과 관계없이 서버와 기기에 비공개로 보존됩니다.
...
- [ ] 3C 운영 전환 — 사용자별 호출 한도·모더레이션·감사 로그와 비용 상한
