...
- **공개 범위 선택** — 완료 기록은 `나만 보기` 또는 `전체 공개`로 저장하고, 공유 상세는 읽기 전용이며 수정·삭제는 작성자에게만 허용
...
현재 외부 베타에서는 이메일 로그인, 취향 설정, 실시간 추천·검색, 영화 상세, 보고 싶어요 저장, `나만 보기`/`전체 공개` Light/Core 감상 기록, 월 캘린더 아카이브와 취향 리포트를 사용할 수 있습니다. Gemini 기반 Core 연계 질문과 Q5 답변 기반 리뷰 초안에는 사용자별 호출 제한·안전 필터·30일 감사 메타데이터가 적용되어 있습니다. 작성 중인 초안은 공개 선택과 관계없이 서버와 기기에 비공개로 보존되며, 내보내기와 이중 확인 계정 삭제 흐름도 제공합니다.
...
- [x] 4단계 웹 핵심 — 캘린더 아카이브, 취향 리포트, 공유·내보내기·삭제 UI와 출시 체크리스트
- [x] 4단계 외부 웹 연결 — 계정 삭제 함수 운영 배포와 반응형 beta 갱신
- [ ] 4단계 네이티브 출시 — TestFlight와 정식 출시 점검
