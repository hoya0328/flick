...
1단계 기반·온보딩과 2단계 영화 발견을 완료했다. 2.5단계의 주문형 전체 상세 구조와 용량 감사도 구현했으며, 전체 경량 색인의 실제 적재만 비용·용량 결정 전 보류했다. 앱은 TMDB를 서버 함수로 호출하고 응답을 Supabase에 캐시하며 공급자 장애 시 기존 캐시와 상태 안내로 복구한다.

## 다음 권장 작업

1. 전체 경량 색인은 DB 용량 또는 외부 검색 인프라를 결정한 뒤 적재한다.
2. Preview 링크는 보안 비공개 주소가 아니므로 제한 테스트가 필요하면 이메일 허용 목록 또는 초대 코드를 구현한다.
3. 그다음 구현은 `기능 개발: 3단계 핵심 기록과 AI`다.
