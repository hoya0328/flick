...
1단계 기반·온보딩, 2단계 영화 발견, 2.5단계 주문형 전체 상세 구조와 3A·3B 핵심 기록을 완료했다. 전체 경량 색인의 실제 적재와 AI 보조 3C만 각각 비용·용량 결정 전 보류했다.

## 다음 권장 작업

1. 전체 경량 색인은 DB 용량 또는 외부 검색 인프라를 결정한 뒤 적재한다.
2. Preview 링크는 보안 비공개 주소가 아니므로 제한 테스트가 필요하면 이메일 허용 목록 또는 초대 코드를 구현한다.
3. 그다음 구현은 `기능 개발: 3C AI 기록 보조`다. API 공급자·월 비용 상한·보관 정책을 먼저 확정한다.
