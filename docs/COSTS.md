...
- Core 리뷰 초안은 Q5 완료 뒤 사용자가 요청할 때 Gemini 호출 1회를 추가한다. 무료 할당량 초과나 오류 시 기존 Q/A와 자유 감상을 보존하며 자동 재시도·자동 과금하지 않는다.
- 무료 등급 입력과 출력은 Google의 제품 개선 및 검토에 사용될 수 있으므로 만 18세 이상 테스트 동의와 개인정보·민감정보 입력 금지를 화면에 표시한다.
- 서버는 연계 질문을 사용자당 하루 20회·분당 6회, 리뷰 초안을 하루 5회·분당 2회로 제한한다. 초과·공급자 무료 한도 도달 시 호출하지 않거나 안전 질문으로 복구한다.
- AI 감사 정보는 본문 없이 기능·상태·오류·모델·토큰·소요시간만 최대 30일 보관한다.
- Google Cloud 프로젝트에는 결제를 연결하지 않았고 유료 전환은 별도 승인을 받는다.
